 ...
 * @author Hp
 */
public class AdministratorUpdateStorageDetailsSceneController implements Initializable {
    ...
    private void saveButtonOnClick(ActionEvent event) {
        ...
        a.setContentText("Are you sure you want to save the storage detail?");
                
        Optional<ButtonType> result = a.showAndWait();
        if(result.get() == ButtonType.OK){
            //you need to write the code to perform the actual task
            Administrator admin = new Administrator();
            admin.updateStorageDetails(produceNameTextField.getText(), weightTextField.getText(), Integer.parseInt(productIdTextField.getText()));
            showCustomInformationAlert("Storage detail updated.");
        }
        else{ 
            //show appropriate cancellation message
            showCustomInformationAlert("Failed to update storage"); 
        }
